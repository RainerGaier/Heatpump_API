...
class HeatPumpCascadeEconTrans(HeatPumpCascadeBase):
    """Two stage transcritical cascading heat pump with two refrigerants and open or closed economiser."""
    ...
    def generate_components(self):
        ...
        self.comps['merge1'] = Merge('High Temperature Economiser Injection')
        self.comps['LT_comp2'] = Compressor('Low Temperature Compressor 2')
        self.comps['HT_comp1'] = Compressor('High Temperature Compressor 1')
        self.comps['merge2'] = Merge('Low Temperature Economiser Injection')
        ...
        if self.econ_type.lower() == 'closed':
            ...
            self.comps['econ1'] = HeatExchanger('Low Temperature Economiser')
            self.comps['econ2'] = HeatExchanger('High Temperature Economiser')
        elif self.econ_type.lower() == 'open':
            self.comps['econ1'] = DropletSeparator('Low Temperature Economiser')
            self.comps['econ2'] = DropletSeparator('High Temperature Economiser')
        else:
            raise ValueError(
    ...
    def check_consistency(self):
        ...
        self.check_expansion_into_vapor_liquid_region(
            conn='A3', p=self.p_evap2, wf=self.wf2, pr=self.params['econ2']['pr2']
        )
        self.check_expansion_into_vapor_liquid_region(
            conn='D3', p=self.p_evap1, wf=self.wf1, pr=self.params['econ1']['pr2']
        )

        if 'econ_type' in self.__dict__.keys():
            if self.econ_type == 'closed':
                self.check_expansion_into_vapor_liquid_region(
                    conn='A10', p=self.p_mid2, wf=self.wf2, pr=self.params['econ2']['pr2']
                )
                self.check_expansion_into_vapor_liquid_region(
                    conn='D10', p=self.p_mid1, wf=self.wf1, pr=self.params['econ1']['pr2']
                )

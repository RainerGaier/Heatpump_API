    ...
    def calc_cost(self, ref_year, current_year, k_evap=1500, k_cond=3500,
        ...
        for complabel in self.nw.comps.index:
            ...
            if comptype == 'Compressor':
                ...
                if 'Evaporator' in complabel or 'Economiser' in complabel:
    ...
    def plot_partload_char(self, partload_char, cmap_type='', cmap='viridis',
        ...
        if cmap_type == 'T_cons_ff':
            ...
            for T_hs_ff in T_hs_ff_range:
                ...
                cbar.set_label('Sink temperature in $°C$')
                ax.set_xlim(0, partload_char['P'].max() * 1.05)
                ax.set_ylim(0, partload_char['Q'].max() * 1.05)
                ax.set_xlabel('Electrical power $P$ in $MW$')
                ax.set_ylabel("Heat flow $\\dot{{Q}}$ in $MW$")
                ax.set_title(f'Source temperature: {T_hs_ff:.0f} °C')
                figs[T_hs_ff] = fig
                axes[T_hs_ff] = ax
        ...
        if cmap_type == 'COP':
            ...
            for T_hs_ff in T_hs_ff_range:
                ...
                cbar.set_label('Performance value $COP$')

                ax.grid()
                ax.set_xlim(0, partload_char['P'].max() * 1.05)
                ax.set_ylim(0, partload_char['Q'].max() * 1.05)
                ax.set_xlabel('Electrical power $P$ in $MW$')
                ax.set_ylabel('Electrical power $\\dot{{Q}}$ in $MW$')
                ax.set_title(f'Source temperature: {T_hs_ff:.0f} °C')
                figs[T_hs_ff] = fig
                axes[T_hs_ff] = ax
        ...
        if cmap_type == 'epsilon':
            ...
            for T_hs_ff in T_hs_ff_range:
                ...
                cbar = plt.colorbar(scatterplot, ax=ax)
                cbar.set_label('Exergy Efficiency $ε$')
            ...
            for label in labels:
                ...
                if comp == 'Compressor':
                    ...
                    # For cascade model's Intermediate heat exchanger
                        ...
                        # For models with evaporator and economiser
                        object.set_attr(

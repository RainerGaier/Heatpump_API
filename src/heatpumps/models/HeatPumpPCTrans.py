...
class HeatPumpPCTrans(HeatPumpBase):
    """Heat pump with open or closed economiser and parallel compression."""
    ...
    def generate_components(self):
        ...
        self.comps['merge'] = Merge('Economiser Injection')

        if self.econ_type.lower() == 'closed':
            self.comps['split'] = Splitter('Condensate Splitter')
            self.comps['econ'] = HeatExchanger('Economiser')
        elif self.econ_type.lower() == 'open':
            self.comps['econ'] = DropletSeparator('Economiser')
        else:
            raise ValueError(
    ...
    def check_consistency(self):
        ...
        self.check_expansion_into_vapor_liquid_region(
            conn='A3', p=self.p_evap, wf=self.wf
        )
        if self.econ_type == 'open':
            ...
            self.check_expansion_into_vapor_liquid_region(
                conn='A10', p=self.p_mid, wf=self.wf
            )
